...
def test_object_creation():
    assert RedisQueueList('mq.test')


def test_push_to_one_queue():
    qnames = 'mq.test.1'
    r = RedisQueueList(qnames)
    r.delete_queues()

    msg = 'string msg'
    response = r.push([msg])
    assert response == ((qnames, 1),)
    assert r._redis.llen(qnames) == 1
    assert r._redis.lindex(qnames, 0) == b'string msg'
...
def test_push_to_one_queues_inited_list_qnames():
    ...
    assert r._redis.llen(qnames[0]) == 1
    assert r._redis.lindex(qnames[0], 0) == b'string msg'
...
def test_push_to_two_queues():
    ...
    assert r._redis.llen(qnames[0]) == 1
    assert r._redis.lindex(qnames[0], 0) == b'string msg'
    assert r._redis.llen(qnames[1]) == 1
    assert r._redis.lindex(qnames[1], 0) == b'string msg'
...
def test_pop_when_two_queues():
    ...
    assert r._redis.llen(qnames[0]) == 0
    assert r._redis.llen(qnames[1]) == 1
...
def test_pop_when_two_queues_but_only_last_queue_has_one():
    ...
    assert r._redis.llen(qnames[1]) == 0
