...
class RedisQueueSet(RedisQueue):
    ...
    def push(self, serialized_msgs):
        ...
        if self._is_multiq:
            ...
            for _qname in self._qnames:
                pipeline.sadd(_qname, *serialized_msgs)
            res = pipeline.execute()
            ret_val = tuple([(q, qsize) for q, qsize in zip(self._qnames, res)])
        else:
            qlenth = self._redis.sadd(self._qnames, *serialized_msgs)
    ...
    def pop(self):
        ...
        if self._is_multiq:
            for _qname in self._qnames:
                _serialized_msg = self._redis.spop(_qname)
                if _serialized_msg is not None:
                    return (_qname, _serialized_msg)
        else:
            _serialized_msg = self._redis.spop(self._qnames)
            if _serialized_msg is not None:
